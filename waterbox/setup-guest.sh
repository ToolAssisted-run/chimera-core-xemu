...
[binaries]
# the wrappers written below: they inject the musl specs exactly once. Meson
# compiles and links its sanity check in ONE gcc call with c_args and
# c_link_args together, and a specs file given twice is a fatal error
# ("attempt to rename spec 'cpp_options'"), so the specs cannot live in
# either list.
c = '$root/build/guest-cc'
cpp = '$root/build/guest-cxx'
ar = 'ar'
strip = 'strip'
pkg-config = '$root/build/guest-bin/x86_64-chimera-linux-musl-pkg-config'
...
[built-in options]
c_args = ['-fvisibility=hidden', '-mcmodel=large', '-mstack-protector-guard=global', '-fno-stack-protector', '-fno-pic', '-fno-pie', '-fcf-protection=none', '-DCHIMERA_GUEST', '-DPIXMAN_NO_TLS', '-I$mb/extern/emulibc', '-I$mb/source/guest/include']
cpp_args = ['-fvisibility=hidden', '-mcmodel=large', '-mstack-protector-guard=global', '-fno-stack-protector', '-fno-pic', '-fno-pie', '-fcf-protection=none', '-fexceptions', '-DCHIMERA_GUEST', '-I$mb/extern/emulibc', '-I$mb/source/guest/include', '-I$sr/include/c++/$gccver', '-I$sr/include/c++/$gccver/x86_64-linux-musl']
c_link_args = []
cpp_link_args = []
EOF
...
# ---- the guest's own dependencies -------------------------------------------
# configure wants glib, zlib and pixman from the guest deps prefix, and only
# from there (the cross file confines pkg-config to it). They are built by
# build-deps.sh against the cross file just written, so a clean checkout gets
# them here rather than failing in configure with "glib-2.0 not found".
[ -f "$deps/lib/pkgconfig/glib-2.0.pc" ] && [ -f "$deps/lib/pkgconfig/pixman-1.pc" ] && [ -f "$deps/lib/pkgconfig/zlib.pc" ] \
	|| sh "$here/build-deps.sh"
