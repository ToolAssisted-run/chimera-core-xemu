/* CHIMERA: deterministic powf/pow for the audio path, taken verbatim from
 * musl (ARM optimized-routines, MIT licence - see the musl COPYRIGHT file).
 * The VP computes sample values with powf/pow; glibc (native reference) and
 * musl (guest) round a handful of results one ULP apart - invisible in
 * machine state, fatal to the audio gate leg's byte-for-byte compare. Both
 * builds compile THIS translation unit instead, so the samples match
 * exactly. Everything is static except chimera_powf/chimera_pow; the data
 * tables are private copies so the guest link does not collide with musl's
 * own.
 *
 * Generated from musl sources by waterbox/gen-det-pow.py; edit that, not
 * this.
 */
#include <math.h>
#include <stdint.h>
...
float chimera_powf(float x, float y);
double chimera_pow(double x, double y);
...
static float __math_xflowf(uint32_t sign, float y)
{
...
/* Returns 0 if not int, 1 if odd int, 2 if even int.  The argument is
   the bit representation of a non-zero finite floating-point value.  */
static inline int checkint64(uint64_t iy)
{
	int e = iy >> 52 & 0x7ff;
	if (e < 0x3ff)
		return 0;
...
static inline int zeroinfnan64(uint64_t i)
{
...
double chimera_pow(double x, double y)
{
	...
	if (predict_false(topx - 0x001 >= 0x7ff - 0x001 ||
		...
		if (predict_false(zeroinfnan64(iy))) {
			if (2 * iy == 0)
		...
		if (predict_false(zeroinfnan64(ix))) {
			double_t x2 = x * x;
			if (ix >> 63 && checkint64(iy) == 1)
				x2 = -x2;
			/* Without the barrier some versions of clang hoist the 1/x2 and
			   thus division by zero exception can be signaled spuriously.  */
